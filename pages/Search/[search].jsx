    ...
    if( !loading && data.total_pages == 0 ) {
        return (
            <ResultsControl resultstate={resultstate} setResultsState={setResultsState} setPage={setPage}>
                <h1 className="w-full my-16 px-8 text-white">There are no movies that matched your query.</h1>
             </ResultsControl>
        )
    }
                ...
                <div className="flex flex-col justify-center my-16">
                    <div className=" w-full px-10 flex flex-col gap-2 ">
                        {data.results.map(result =>{
                            return(
                                <Link key={result.id} href={resultstate ==='movie' ? `/Movies/${result.id}` :`/TvShow/${result.id}` } className='flex flex-row  bg-[#121212] rounded-lg ' passHref>
                                {result.poster_path ? <Image width={200} className='w-[120px] h-full object-cover sm:rounded-l-lg  ml-0' height={300} src={`https://www.themoviedb.org/t/p/w220_and_h330_face${result.poster_path}`} alt={result.original_title} /> :  <Image width={200}  className='bg-slate-400 w-[120px] h-[150px] rounded-l-lg' height={300} src='https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-38-picture-grey-c2ebdbb057f2a7614185931650f8cee23fa137b93812ccb132b9df511df1cfac.svg' alt={result.original_title} />}
