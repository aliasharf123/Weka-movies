...
function Movies() {
    ...
    const {data , loading } = useFetch(`https://api.themoviedb.org/3/discover/movie?language=en-US&sort_by=${sort}&include_adult=false&include_video=false&page=${page}&with_watch_monetization_types=flatrate${genere}`)
    
    return ( 
        <MoviesContext.Provider value={{setSort , setPage ,languge , setLanguges , setGenere , genere}}>
            ...
            {(style )?
                        ...
                        <div className=" w-full sm:px-10 px-0 flex flex-col gap-2 ">
                            {data.results.map(result =>{
                                return(
                                    ...
                                    {result.poster_path ? <Image width={200} className='w-[100px] h-full object-cover sm:rounded-l-lg  ml-0' height={300} src={`https://image.tmdb.org/t/p/w780${result.poster_path}`} alt={result.original_title} /> :  <Image width={200}  className='bg-slate-400 w-[100px] h-[150px] rounded-l-lg' height={300} src='https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-38-picture-grey-c2ebdbb057f2a7614185931650f8cee23fa137b93812ccb132b9df511df1cfac.svg' alt={result.original_title} />}
                ...
                <div className="flex justify-center flex-col">
                    <div className='grid md:grid-cols-5 grid-cols-2 w-full gap-3   sm:px-10 px-0  mb-5 mt-16 ' >
