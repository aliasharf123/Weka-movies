...
export default async function Videos({ url }: { url: string }) {
  const data = await queryClient<Content>(url, () =>
        ...
        return (
          ...
          >
            <Link
              ...
              className="cursor-pointer  relative  md:hover:scale-105 duration-300 flex justify-center items-center"
              scroll={false}
            >
              {movie.backdrop_path && (
                <Image
              ...
              <PlayCircleOutlined className="absolute text-white text-3xl md:group-hover:scale-125 duration-200" />
